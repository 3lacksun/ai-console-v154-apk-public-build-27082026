...
export function sha256Bytes(input) {
  ...
  const out=new Uint8Array(32); const outView=new DataView(out.buffer); [h0,h1,h2,h3,h4,h5,h6,h7].forEach((v,i)=>outView.setUint32(i*4,v,false)); return out;
}
export const bytesToHex = (bytes) => Array.from(bytes, v => v.toString(16).padStart(2,'0')).join('');
export const sha256Hex = (input) => bytesToHex(sha256Bytes(input));
