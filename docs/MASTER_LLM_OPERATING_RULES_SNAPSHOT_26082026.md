...
**Rules-Version:** 1.2.18  
**Revision:** 20260826-02  
...
-   `VER-012` Master Documentation releases that distribute the canonical
    ...
    execution. A self-test code/logic failure is a release blocker. Where
    `pdo_sqlite` is unavailable, the self-test must exercise the authorised
    `HOST-012` structured JSON persistence fallback rather than skipping database
    initialisation. Only when neither the preferred SQLite path nor the authorised
    fallback can be executed may the affected persistence coverage remain PARTIAL
    or UNVERIFIABLE rather than PASS.
...
-   `HAND-008` When a new project-controlled handover ZIP is created, finalised,
    verified to the extent required by the applicable release process, and
    persisted successfully, that handover becomes the project's current main
    continuation version and authoritative working baseline. It supersedes prior
    handover ZIPs and earlier implementation/release baselines for subsequent
    continuation work unless the user or an authoritative project specification
    explicitly designates the new handover as non-authoritative, provisional,
    branch-specific, or otherwise preserves another baseline. Superseded artefacts
    remain historical provenance and must not be selected as the main version merely
    because they are newer by timestamp, larger, or more recently discovered.
...
-   `HOST-012` For applications using the canonical shared-hosting persistence
    profile, or otherwise required by their authoritative specification to remain
    operational in a constrained managed execution environment, if native
    `pdo_sqlite` support is unavailable and installation of that extension is
    blocked, automatically use the authorised structured local JSON flat-file
    persistence layer so state persistence and core workflow verification can
    continue without failing solely because the database extension is missing.
    This fallback remains subordinate to an explicit project-specific persistence
    architecture under `AUTH-002`/`AUTH-009`; an explicitly SQLite-only project is
    not silently redesigned, and the unavailable SQLite runtime gate is instead
    reported truthfully as PARTIAL or UNVERIFIABLE.
...
## AA. CONVERT authoritative Android conversion suitability skill

-   `CONVERT-001` The exact trigger phrase `skill convert` activates the canonical Android conversion-suitability audit and planning workflow.
-   `CONVERT-002` The canonical skill definition is `/Skills/convert/SKILL.md`. The current stable canonical version is CONVERT v1.0.0 unless explicitly superseded by an authorised successor.
-   `CONVERT-003` CONVERT is a whole-application, read-only assessment whose primary purpose is to decide whether an existing application should remain on its current architecture or be converted to a stronger Android delivery architecture. Conversion is not presumed to be beneficial.
-   `CONVERT-004` Before scoring conversion paths, CONVERT must reconstruct the authoritative application baseline and cumulative active requirements from the available Build Specification, Technical Specifications, Feature Locks, authorised amendments, current handover/release evidence and explicit current user instructions.
-   `CONVERT-005` Every assessment must compare the current architecture/no-conversion option against Capacitor, React Native + Expo, React Native bare/native project, Flutter, native Android using Kotlin + Jetpack Compose + Android SDK + Android Gradle Plugin/Gradle, and a hybrid/partial-native option when materially relevant. A technically impossible candidate remains visible with its disqualification reason.
-   `CONVERT-006` Gradle is an Android build system/toolchain rather than a competing application UI framework. Native Android comparisons must therefore identify the full native architecture, including Kotlin/Jetpack Compose/Android SDK and AGP/Gradle where applicable.
-   `CONVERT-007` The assessment must be application-specific and cover feature preservation, UI and business-logic reuse, backend/API reuse, data migration, Android native pressure, UI/UX gain, performance, offline/local reliability, background execution, filesystem/media/device integration, accessibility, build/release suitability, maintainability, migration complexity, regression risk and long-term architectural fit.
-   `CONVERT-008` Where framework capability, compatibility, support status, Android tooling or plugin/native-module availability is material to the verdict, CONVERT must use current official primary-source evidence where available and record the research date.
-   `CONVERT-009` Candidate scoring must use disclosed application-specific weighting rather than a fixed universal preference. Hard incompatibility with an authoritative requirement disqualifies a candidate regardless of its arithmetic score.
-   `CONVERT-010` The report must include a feature-by-feature conversion matrix and a consolidated candidate comparison covering suitability, changes, retained elements, benefits, pros, cons, native/UX/performance gains, reuse, rewrite, difficulty, risk, maintenance impact, blockers/dependencies, best fit and verdict.
-   `CONVERT-011` CONVERT is read-only by default. Activation authorises discovery, inspection, current-source research, scoring and assessment evidence; it does not authorise application source mutation, conversion implementation, build/signing, deployment or release mutation.
-   `CONVERT-012` After the completed audit, the first decision gate is user approval of the recommended path, a viable alternative, or keeping the current architecture. Approval of a path authorises generation of a strict autonomous implementation plan only; implementation still requires separate authorisation.
-   `CONVERT-013` An approved implementation plan must preserve every active authoritative feature unless explicitly changed by the user and must map source-to-target components, screens/navigation, business logic, backend/API, data/storage migration, native modules/plugins, offline/background behaviour, accessibility, build configuration, dependency resolution, CI/signing, acceptance criteria, rollback, regression and truthful verification gates.
-   `CONVERT-014` CONVERT evidence is subject to the general truthfulness rules: static inspection is not runtime/device verification, a framework's theoretical capability is not proof of successful application implementation, and unexecuted migration/build/device tests must remain PARTIAL or UNVERIFIABLE rather than PASS.
-   `CONVERT-015` CONVERT is an independent decision skill. It may consume evidence from AuditSkill, PER002, Upgrade Advisor, EXEC002 or REM002 where available, but it must verify material claims against the selected current baseline and must not silently alter those skills' locked orchestration contracts.

## AB. MIGRATE authoritative project / LLM migration handover skill

-   `MIGRATE-001` The exact trigger phrase `skill migrate` activates the canonical project/LLM migration-handover preparation workflow. Matching of the trigger phrase is case-insensitive, so `Skill migrate` is equivalent.
-   `MIGRATE-002` The canonical skill definition is `/Skills/migrate/SKILL.md`. The current stable canonical version is MIGRATE v1.0.0 unless explicitly superseded by an authorised successor.
-   `MIGRATE-003` MIGRATE's purpose is to prepare an existing project for reliable continuation in another project folder, workspace or LLM by producing one independently usable handover package containing the most complete authoritative standalone application and current relevant documentation.
-   `MIGRATE-004` MIGRATE operates in Release/Handover Mode. Activation authorises project discovery, baseline selection, documentation reconstruction/generation, packaging, integrity verification and persistence of migration artefacts; it does not authorise application feature redesign, architecture changes, source remediation or application-logic mutation unless separately authorised.
-   `MIGRATE-005` MIGRATE must recursively inventory the complete available project root, including current source/application trees, releases, handovers, archives, build outputs, documentation, specifications, feature locks, amendments, dependency manifests and verification evidence relevant to baseline selection. It must not select a baseline merely because a filename or timestamp is newest.
-   `MIGRATE-006` Baseline selection must prioritise controlling authority and cumulative authorised feature/requirement coverage, then standalone/self-contained operability, verification quality and only then recency where candidates are otherwise equivalent. Historical or superseded artefacts remain evidence/provenance unless explicitly re-authorised.
-   `MIGRATE-007` A migration baseline is disqualified if it requires another release, handover ZIP, patch-only package, prior version, undocumented external project tree or missing cross-version file to function. Project-local shared dependencies may be included only when they are incorporated into the resulting standalone package or resolved through the authorised dependency policy.
-   `MIGRATE-008` Before generating migration documentation, MIGRATE must reconstruct cumulative authorised scope from the original/full Build Specification, Technical Specifications, Feature Locks, authorised amendments/change-control decisions, persisted user requests/decisions, current handover/release evidence and explicit current user instructions. Later authorised changes override conflicting earlier requirements; non-conflicting earlier requirements survive unless explicitly retired.
-   `MIGRATE-009` MIGRATE must generate a consolidated current Technical Specification document when one complete current document does not already exist. It must describe the authorised target and architecture rather than rewriting requirements to match defects, omissions or limitations in the selected implementation.
-   `MIGRATE-010` MIGRATE must preserve requirement lineage through a traceability record that maps material active requirements to their controlling source and, where evidence permits, to the selected application implementation and verification state. Missing, contradictory or unresolved requirements must remain explicit.
-   `MIGRATE-011` The migration handover ZIP must contain, at minimum, the complete selected standalone application/project tree, the consolidated Technical Specification, current relevant authoritative source documentation, a handover document, a strict receiving-LLM handover prompt, release/package metadata and sufficient migration evidence for independent continuation. A current Master-document continuity snapshot must be included when the authoritative Library source is available.
-   `MIGRATE-012` The receiving-LLM handover prompt must be self-contained, assume no prior chat history, identify the selected baseline and governing documents, require the receiving LLM to bootstrap from the authoritative Master Rules when available, preserve authorised requirements and verification truthfulness, and forbid dependency on older project releases not contained in or explicitly resolved by the handover.
-   `MIGRATE-013` MIGRATE packages are classified as `HANDOVER` and are subject to the Master ZIP/handover rules, including a `DDMMYYYYHHMMSS` timestamp in the project-controlled ZIP filename and inclusion of at least one complete authoritative specification document.
-   `MIGRATE-014` Before a migration package is designated ready, perform clean re-extraction and applicable archive-integrity, inventory, release-identity, documentation-consistency, secret-leakage and self-containment checks. Unexecuted runtime/device/deployment checks remain PARTIAL or UNVERIFIABLE and are never converted to PASS.
-   `MIGRATE-015` The final verified migration ZIP and final continuation documents must be persisted with the source project's designated project files when such a location exists and persistence is authorised. If the user supplies a target project folder, also place a verified copy of the migration ZIP there without deleting or overwriting the source baseline unless explicitly authorised.
-   `MIGRATE-016` MIGRATE status must distinguish at least `MIGRATION_READY`, `MIGRATION_READY_WITH_EXTERNAL_GATES`, `MIGRATION_PARTIAL`, and `MIGRATION_BLOCKED`. A package is not `MIGRATION_READY` while the selected application is cross-version dependent, mandatory documentation is materially incomplete, clean-package verification failed, or a required persistence operation remains unverified.
-   `MIGRATE-017` Missing optional tools, device/browser execution or external services do not automatically block migration preparation. Continue every meaningful static, structural, documentation and package check; identify exactly which stronger gates remain PARTIAL or UNVERIFIABLE.
-   `MIGRATE-018` MIGRATE is an independent handover-preparation skill. It may consume current evidence from AuditSkill, PER002, CONVERT, Upgrade Advisor, EXEC002 or REM002, but it must verify material baseline claims against the selected current project state and must not silently alter another skill's locked orchestration contract.
-   `MIGRATE-019` MIGRATE is complete only when the most complete authoritative standalone baseline supported by evidence has been selected, cumulative scope has been reconstructed, current continuation documentation and receiving-LLM prompt have been generated, the handover ZIP has been clean-verified as independently usable, required persistence has been verified, and no dependency on an older release or undocumented conversation history is required for continuation.


## AC. GITPREP approved GitHub repository and Android build-preparation skill

-   `GITPREP-001` The exact trigger phrase `skill gitprep` activates the approved canonical GitPrep repository-engineering, Android CI/CD, APK-build-verification and release-preparation workflow.
-   `GITPREP-002` The canonical skill definition is `/Skills/gitprep/SKILL.md`. The approved stable canonical version is GITPREP v1.0.0 unless explicitly superseded by an authorised successor.
-   `GITPREP-003` GitPrep is an execution skill rather than a report-only checklist. Activation authorises inspection and locally actionable repository preparation/remediation within the supplied project workspace, including GitHub support files, CI configuration, build configuration, documentation and verification evidence, subject to the authoritative project specification and general mutation boundaries.
-   `GITPREP-004` Before substantive repository modification, GitPrep must establish the authoritative project baseline and reconcile current explicit user instructions, Build Specifications, Technical Specifications, Feature Locks, authorised amendments, current handover/release identity and implementation evidence. Superseded or historical material remains provenance unless re-authorised.
-   `GITPREP-005` When internet/documentation access exists, current GitHub-, Android-, Gradle- and framework-specific recommendations that materially affect generated configuration must be checked against current official primary documentation. If current documentation cannot be accessed, the affected currency claim remains `UNVERIFIABLE`; URLs, Action versions, Action SHAs and compatibility claims must never be invented.
-   `GITPREP-006` GitPrep must detect the project's actual framework, languages, package manager, dependency model, Android build path, application module and existing CI/Git state before choosing commands or repository structure. It must not arbitrarily migrate frameworks or flatten a valid project structure.
-   `GITPREP-007` Repository inventory must classify source, build configuration, dependency manifests/lockfiles, tests, documentation, generated outputs, machine-specific files, large binaries, sensitive/signing material, duplicates, historical files and unclear files. Massive generated/dependency trees should be inventoried before deep inspection and parsed internally only when relevant to a concrete issue.
-   `GITPREP-008` Existing Git history, branches, remotes, tags, upstream state, submodules, Git LFS configuration and staged/working-tree state must be detected before Git initialisation or remote commands. GitPrep must not blindly run `git init`, replace an existing remote, destroy history or assume a new repository.
-   `GITPREP-009` Secret review must cover current readable files and, when an existing Git history is available, material historical exposure. Removing a credential from the working tree does not remove it from history; exposed credentials should be rotated where appropriate, and published-history rewriting requires explicit authorisation.
-   `GITPREP-010` Dependency lockfiles and project-selected package managers must be preserved where appropriate. Missing dependencies follow `DEP-001`; GitPrep must not silently substitute package managers, dependency versions, Android SDK levels, AGP, Gradle, Kotlin or Java merely because newer versions exist.
-   `GITPREP-011` For native/Gradle Android projects, validate actual AGP/Gradle/JDK/Kotlin/SDK compatibility, modules, manifests, build variants/flavours, signing configuration and Gradle Wrapper completeness. Wrapper integrity review should include the distribution URL, `distributionSha256Sum` where appropriate, executable state and Wrapper JAR validation where tooling permits.
-   `GITPREP-012` Non-native Android workflows must follow the detected framework. Expo projects must distinguish Continuous Native Generation/Prebuild, committed native directories, EAS Build and local native builds; absence of `android/` is not a defect for a valid CNG project, and an AAB must not be reported as fulfilment of an APK requirement.
-   `GITPREP-013` Repository support files such as README, `.gitignore`, `.gitattributes`, CI workflows, Dependabot, CODEOWNERS, issue/PR templates, security/contribution documentation and environment examples must be created or modified only when applicable to the actual repository. Do not invent licences, owners, security contacts, credentials, release history or meaningless boilerplate.
-   `GITPREP-014` GitHub Actions must use least privilege, current supported workflow syntax and project-appropriate build commands. Third-party/reusable Actions should use immutable full-commit-SHA pinning where current official guidance and practical verification support it; never invent an Action SHA. Privileged `pull_request_target` jobs must not execute untrusted PR-head code merely to gain secrets or write authority.
-   `GITPREP-015` When execution facilities exist, GitPrep must run applicable static checks/tests and the actual Android build, remediate locally actionable failures where authorised, then rerun failed and directly affected regression checks. A configured workflow or plausible Gradle file is not proof of a successful build.
-   `GITPREP-016` APK verification requires locating the actual generated APK file or files and recording applicable module, variant/flavour/ABI or split, exact path, filename, size and debug/release classification. SHA-256 and APK signature verification should be executed where tooling permits; signature state must not be inferred from a filename alone.
-   `GITPREP-017` Production Android signing identities, keystores and signing secrets remain sensitive and must not be committed or exposed. Debug/test signing must be distinguished from production release signing, and a production signing identity must not be generated or replaced without explicit authority.
-   `GITPREP-018` Repository administration settings outside source control, including Actions permissions, environments, rulesets, branch protection, secret-scanning features and release-security controls, must be distinguished from committed repository files. GitPrep must not claim an administrative setting changed unless it actually changed.
-   `GITPREP-019` GitPrep activation does not itself authorise remote GitHub mutation. Remote source pushes, force pushes, tag pushes, GitHub Release publication, remote deletion or comparable external mutation require explicit current user authorisation; before an authorised remote write, inspect the target state and verify the resulting remote state afterward.
-   `GITPREP-020` Final repository acceptance should use clean-room/fresh-checkout build verification where facilities permit. If GitPrep creates a transferable repository ZIP, classify it under the Master package rules, exclude secrets/caches/unneeded generated output, preserve required source and authoritative documentation, and perform archive/content/integrity checks before designating it ready.
-   `GITPREP-021` GitPrep verification uses the general PASS / FAIL / PARTIAL / UNVERIFIABLE / NOT APPLICABLE truth model and keeps issue severity/category separate from verification state. Static inspection, intended commands, generated YAML, predicted APK paths and unexecuted tests are never PASS evidence for runtime/build claims.
-   `GITPREP-022` GitPrep is complete only when every locally actionable repository/build defect in the authorised scope has been resolved and appropriately reverified or truthfully left behind a genuine external gate, the repository state is maintainable and reproducible from the documented instructions, and any produced package/persistence claim has been independently checked.
