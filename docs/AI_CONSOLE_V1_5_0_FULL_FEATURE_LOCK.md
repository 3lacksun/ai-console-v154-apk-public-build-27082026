# Dr Stones Command Centre v1.5.0 — Full Feature Lock
...
## v1.5.0 locked additions — Command Intelligence

The preceding full feature lock remains cumulative and active. The following additions are now locked for v1.5.0:

- **FL-INT-001 — Four-destination preservation:** Chats, Workspaces, Documents and Settings remain the only primary destinations; Command Intelligence is integrated globally and per-workspace.
- **FL-MEM-001 — Workspace Memory:** persistent workspace-isolated memories with create/edit/delete, pin, enable, provenance, bounded relevance injection and visible context semantics.
- **FL-MEM-002 — Memory privacy boundary:** no implicit cross-workspace memory injection; protected secrets are not memories or archive content.
- **FL-SKL-001 — Skills:** reusable versioned workflows with ordered steps, conditions, template variables, workspace scope, run history and chat/document/memory/none output routing.
- **FL-SKL-002 — Built-in starter Skills:** Workspace Brief, Memory Curator and Weekly Review.
- **FL-USG-001 — Usage telemetry:** generation ledger spanning chat, voice, Skills, tasks and document AI operations, with model/provider/request provenance.
- **FL-USG-002 — Cost truthfulness:** provider-reported cost is displayed as such; absent provider cost remains unavailable and is never fabricated.
- **FL-TSK-001 — Scheduled Tasks:** persistent once/interval/daily/weekly tasks with conditions, run-now, pause/resume and history.
- **FL-TSK-002 — Scheduling boundary:** app-active execution plus launch/resume catch-up is locked; closed-app OS background execution is not part of v1.5.0 acceptance unless separately authorised and implemented.
- **FL-VOI-001 — Full Voice Mode:** listen → finalise → generate → speak conversational flow with auto-send, auto-listen, selectable voice, playback rate, interruption and text fallback.
- **FL-VOI-002 — Android speech regression protection:** retain the narrowly scoped v1.4.3 manual-stop client/code-5 transcript fallback.
- **FL-ARC-001 — Intelligence-aware workspace archives:** archive schema v3 includes memory/Skills/usage/tasks histories, remaps imported IDs and imports scheduled tasks paused.
- **FL-DATA-001 — Storage schema v5:** deterministic migration preserves prior state and seeds built-in Skills only when required.

No v1.4.x locked feature is retired by this section.
