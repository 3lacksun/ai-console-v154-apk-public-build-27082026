...
assert(streamChat.includes('fetchTogetherSonicVoices')&&streamChat.includes('createTogetherSonicSpeech')&&streamChat.includes("response_format: 'mp3'")&&streamChat.includes('fetchProviderWithRetry')&&streamChat.includes('shouldRetryAttempt'),'Together Sonic voice/speech client or bounded retry policy missing');
assert(fs.existsSync(path.join(root,'src/providers/retryPolicy.mjs'))&&fs.existsSync(path.join(root,'tests/retryPolicy.test.mjs'))&&retryPolicy.includes('maxAttempts: 3')&&retryPolicy.includes('isRetryableProviderFailure'),'provider retry regression files missing');
assert(generationState.includes("PAUSED: 'PAUSED'")&&generationManager.includes('pause(chatId)')&&generationManager.includes('resume(chatId)')&&streamChat.includes('bufferedDeltas')&&source.includes('Pause Response')&&source.includes('Resume Response'),'pause/resume streaming contract missing');
assert(conversationSchema.includes('aiPreset')&&privacy.includes('safeAiPreset')&&source.includes('saveChatAiPreset')&&source.includes('Save chat AI'),'per-conversation AI preset contract missing');
assert(source.includes('togetherSonicCacheRef')&&source.includes('clearTogetherSonicCache')&&settings.includes('Clear temporary Sonic audio cache'),'temporary Sonic audio cache control missing');
assert(source.includes('Keyboard.addListener')&&source.includes('androidKeyboardHeight')&&source.includes('setTimeout(scrollToBottom, 180)'),'Android keyboard-safe composer contract missing');
assert(settings.includes("from '@react-native-picker/picker'")&&settings.includes('Together Sonic voice')&&settings.includes('Refresh voices'),'Together Sonic voice dropdown missing');
assert(bubble.includes('Speak with Together Sonic')&&bubble.includes('IconSpeaker')&&bubble.includes('isSpeaking'),'visible Together Sonic speech-bubble control missing');
...
assert(workflow.includes('default: true')&&workflow.includes("github.event_name != 'workflow_dispatch' || inputs.run_emulator_checks")&&workflow.includes('Release runtime acceptance gate')&&workflow.includes('publish_diagnostic_artifact')&&workflow.includes('This is not release acceptance evidence.')&&workflow.includes("env.RUN_EMULATOR_CHECKS == 'true' || env.PUBLISH_DIAGNOSTIC_ARTIFACT == 'true'")&&workflow.includes('not published when disabled unless the separately labelled diagnostic artifact option is explicitly selected'),'runtime emulator/16-KB release acceptance must default on; bypassed runtime checks may publish only an explicitly selected, clearly labelled diagnostic artifact');
...
for(const retired of ['src/components/PinGateModal.js','src/security/pinThrottle.mjs','src/utils/pinKeypad.mjs','src/utils/pinVerifier.mjs','src/utils/settingsPolicy.mjs','tests/pinKeypad.test.mjs','tests/pinVerifier.test.mjs','tests/settingsPolicy.test.mjs']) assert(!fs.existsSync(path.join(root,retired)),`retired PIN implementation remains active: ${retired}`);
