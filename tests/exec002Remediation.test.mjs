...
test('current handover checksum manifests are self-consistent', () => {
  ...
  for (const name of ['HANDOVER_SHA256SUMS.txt', 'SHA256SUMS_EXEC002.txt']) {
    const lines = fs.readFileSync(path.join(root, name), 'utf8').split(/\r?\n/).filter(Boolean);
    assert.ok(lines.length > 0, `${name} must not be empty`);
    for (const line of lines) {
      const match = line.match(/^([a-f0-9]{64})  (.+)$/);
      assert.ok(match, `invalid checksum line in ${name}: ${line}`);
      const rel = match[2].replace(/^\.\//, '');
      const target = path.join(root, rel);
      assert.ok(fs.existsSync(target), `${name} references missing file: ${rel}`);
      const actual = crypto.createHash('sha256').update(fs.readFileSync(target)).digest('hex');
      assert.equal(actual, match[1], `${name} checksum mismatch: ${rel}`);
    }
  }
});
