...
import { AccessibilityInfo, Alert, AppState, BackHandler, Keyboard, KeyboardAvoidingView, PermissionsAndroid, Platform, ScrollView, StyleSheet, Text, TextInput, TouchableOpacity, View, FlatList, useWindowDimensions } from 'react-native';
...
import DeviceLockScreen from './src/components/DeviceLockScreen';
...
import { DEFAULT_SYSTEM_PROMPT, clearLegacyAppLockRecord, commitStateTransaction, formatProviderName, getApiKeyResult, getAppLockSettings, getTogetherApiKeyResult, getJSON, getVersionedAppStateResult, INITIAL_MODELS, persistAndVerifyVersionedAppState, setApiKey as persistApiKey, setAppLockSettings, setTogetherApiKey as persistTogetherApiKey, setJSON } from './src/utils/storage';
import { sanitizeChatsForPersistence } from './src/utils/privacy.mjs';
import { completeChatCompletion, createTogetherSonicSpeech, fetchModels, fetchTogetherSonicVoices, streamChatCompletion } from './src/utils/streamChat';
import { DEFAULT_PROVIDER_MODELS, DEFAULT_TOGETHER_SONIC_VOICE, INITIAL_PROVIDER_MODEL_GROUPS, TOGETHER_SONIC_TTS_MODEL, normaliseProviderId, providerLabel, ProviderId } from './src/providers/providerRegistry.mjs';
import { APP_LOCK_TIMEOUT_MS, DEFAULT_APP_LOCK_SETTINGS, disableAppLock, isAppLockDue, markAppLockAuthenticated, normaliseAppLockSettings } from './src/security/appLock.mjs';
...
function AIConsoleApp() {
  ...
  const [appLockSettings, setAppLockSettingsState] = useState(DEFAULT_APP_LOCK_SETTINGS);
  const [appLocked, setAppLocked] = useState(false);
  const [isAuthenticatingAppLock, setIsAuthenticatingAppLock] = useState(false);
  ...
  const [togetherSonicVoice, setTogetherSonicVoice] = useState(DEFAULT_TOGETHER_SONIC_VOICE);
  const [togetherSonicVoices, setTogetherSonicVoices] = useState([{ id: DEFAULT_TOGETHER_SONIC_VOICE, name: DEFAULT_TOGETHER_SONIC_VOICE }]);
  const [isRefreshingTogetherSonicVoices, setIsRefreshingTogetherSonicVoices] = useState(false);
  ...
  const appLockSettingsRef = useRef(DEFAULT_APP_LOCK_SETTINGS);
  const appLockPromptActiveRef = useRef(false);
  const togetherSonicPlayerRef = useRef(null);
  const togetherSonicCacheRef = useRef(new Map());
  ...
  useEffect(() => {
    ...
    const hydrate = async () => {
      try {
        ...
        setTogetherSonicVoice(typeof storedTogetherSonicVoice === 'string' && storedTogetherSonicVoice.trim() ? storedTogetherSonicVoice.trim() : DEFAULT_TOGETHER_SONIC_VOICE);
        const restoredAppLock = normaliseAppLockSettings(storedAppLockSettings);
        appLockSettingsRef.current = restoredAppLock;
        setAppLockSettingsState(restoredAppLock);
        setAppLocked(isAppLockDue(restoredAppLock));
        void clearLegacyAppLockRecord();
  ...
  useEffect(() => { if (hydrated && !hydrationDegradedRef.current) setJSON('togetherSonicVoice', togetherSonicVoice); }, [togetherSonicVoice, hydrated]);
  useEffect(() => {
    if (Platform.OS !== 'android') return undefined;
    const onKeyboardShow = (event) => {
      setAndroidKeyboardHeight(Math.max(0, Number(event?.endCoordinates?.height) || 0));
      requestAnimationFrame(() => listRef.current?.scrollToEnd?.({ animated: true }));
    };
    const onKeyboardHide = () => setAndroidKeyboardHeight(0);
    const show = Keyboard.addListener('keyboardDidShow', onKeyboardShow);
    const hide = Keyboard.addListener('keyboardDidHide', onKeyboardHide);
    return () => { show.remove(); hide.remove(); };
  }, []);
  useEffect(() => {
    void setAudioModeAsync({ playsInSilentMode: true, interruptionMode: 'doNotMix' }).catch(() => {});
    return () => {
      const active = togetherSonicPlayerRef.current;
      togetherSonicPlayerRef.current = null;
      try { active?.subscription?.remove?.(); active?.player?.pause?.(); active?.player?.remove?.(); } catch (_) {}
      const cachedUris = [...togetherSonicCacheRef.current.values()].map((entry) => entry.uri);
      togetherSonicCacheRef.current.clear();
      for (const uri of new Set([active?.uri, ...cachedUris].filter(Boolean))) void FileSystem.deleteAsync(uri, { idempotent: true }).catch(() => {});
    };
  }, []);
  ...
  useEffect(() => {
    const lifecycleSubscription = AppState.addEventListener('change', (state) => {
      if (state === 'active') {
        if (isAppLockDue(appLockSettingsRef.current)) setAppLocked(true);
        return;
      }
  ...
  useEffect(() => {
    if (!hydrated || !appLockSettings.enabled || appLocked) return undefined;
    const remaining = Math.max(0, (appLockSettings.lastAuthenticatedAt + APP_LOCK_TIMEOUT_MS) - Date.now());
    const timer = setTimeout(() => setAppLocked(true), remaining);
    return () => clearTimeout(timer);
  }, [hydrated, appLocked, appLockSettings]);

  useEffect(() => {
    if (!hydrated || !appLocked || !appLockSettings.enabled) return;
    void unlockAppWithDevice();
  }, [hydrated, appLocked, appLockSettings.enabled]);
...
${project}`);
  ...
  const pauseGeneration = () => { if (activeChat && !generationManagerRef.current?.pause(activeChat.id)) setError('This response cannot be paused at its current stage.'); };
  const resumeGeneration = () => { if (activeChat && !generationManagerRef.current?.resume(activeChat.id)) setError('This response cannot be resumed at its current stage.'); };
  const saveChatAiPreset = () => {
    if (!activeChat) return;
    const aiPreset = { provider: activeProvider, model, temperature, maxTokens: normaliseOutputTokens(maxTokens), togetherSonicVoice };
    setConversationState((previous) => ({ ...previous, chats: previous.chats.map((chat) => chat.id === activeChat.id ? { ...chat, aiPreset, updatedAt: Date.now() } : chat) }));
    setError(`Conversation preset saved: ${providerLabel(activeProvider)} · ${model}.`);
  };
  const clearChatAiPreset = () => {
    if (!activeChat?.aiPreset) return;
    setConversationState((previous) => ({ ...previous, chats: previous.chats.map((chat) => chat.id === activeChat.id ? { ...chat, aiPreset: null, updatedAt: Date.now() } : chat) }));
    setError('Conversation preset cleared; this chat now inherits global settings.');
  };

  const persistAppLock = async (next) => {
    const saved = await setAppLockSettings(next);
    appLockSettingsRef.current = saved;
    setAppLockSettingsState(saved);
    return saved;
  };
  const promptForDeviceAuthentication = async () => {
    if (appLockPromptActiveRef.current) return false;
    appLockPromptActiveRef.current = true;
    setAppLockError('');
    setIsAuthenticatingAppLock(true);
    try {
      const result = await LocalAuthentication.authenticateAsync({
        promptMessage: 'Unlock Dr Stones Command Centre',
        promptDescription: 'Use your biometric or device screen lock.',
        cancelLabel: 'Cancel',
      ...
      if (!result.success) {
        const detail = result.error === 'not_enrolled' ? 'Set up a biometric or device screen lock, then try again.' : result.error === 'lockout' ? 'Device authentication is temporarily locked. Use your device screen lock or wait and try again.' : 'Device authentication was cancelled or did not succeed.';
        setAppLockError(detail);
        return false;
      }
      return true;
    } catch (authError) {
      setAppLockError(authError?.message || 'Device authentication is unavailable on this device.');
      return false;
    } finally {
      appLockPromptActiveRef.current = false;
      setIsAuthenticatingAppLock(false);
    }
  };
  const unlockAppWithDevice = async () => {
    if (!appLockSettingsRef.current.enabled) { setAppLocked(false); return; }
    const authenticated = await promptForDeviceAuthentication();
    if (!authenticated) return;
    try {
      await persistAppLock(markAppLockAuthenticated(appLockSettingsRef.current));
      setAppLocked(false);
      setAppLockError('');
    } catch (persistError) {
      setAppLockError(persistError?.message || 'The successful unlock could not be recorded safely.');
    }
  };
  const handleToggleAppLock = async (enabled) => {
    if (enabled !== true) {
      try {
        await persistAppLock(disableAppLock());
        setAppLocked(false);
        setAppLockError('');
      } catch (persistError) { setError(persistError?.message || 'Device lock preference could not be disabled.'); }
      return;
    }
    const authenticated = await promptForDeviceAuthentication();
    if (!authenticated) { setError('Device lock was not enabled because device authentication did not succeed.'); return; }
    try {
      await persistAppLock(markAppLockAuthenticated(appLockSettingsRef.current, Date.now(), true));
      setError('Device lock enabled. The app will relock after six hours.');
    } catch (persistError) { setError(persistError?.message || 'Device lock preference could not be saved.'); }
  };
  const requestProtectedSettingsAccess = () => { if (appLocked || isAppLockDue(appLockSettingsRef.current)) { setAppLocked(true); void unlockAppWithDevice(); return; } setIsLLMSettingsOpen(true); };
  ...
  const startGeneration = (chatId, targetMessageId, apiMessages, options = {}) => {
    const chatPreset = conversationStateRef.current.chats.find((item) => item.id === chatId)?.aiPreset || null;
    const requestProvider = normaliseProviderId(options.provider || chatPreset?.provider || activeProvider);
    const requestModel = options.model || chatPreset?.model || providerModels[requestProvider] || DEFAULT_PROVIDER_MODELS[requestProvider];
    const requestTemperature = options.temperature ?? chatPreset?.temperature ?? temperature;
    const requestMaxTokens = normaliseOutputTokens(options.maxTokens ?? chatPreset?.maxTokens ?? maxTokens);
    ...
    generationManagerRef.current.start({ chatId, targetMessageId, streamFactory: (callbacks) => {
      ...
      const stream = streamChatCompletion({
        provider: requestProvider, apiKey: requestApiKey, model: requestModel, messages: apiMessages, temperature: requestTemperature, maxTokens: requestMaxTokens,
  ...
  const stopTogetherSonicSpeech = async () => {
    const active = togetherSonicPlayerRef.current;
    togetherSonicPlayerRef.current = null;
    if (!active) { setSpeakingMessageId(''); return; }
    try { active.subscription?.remove?.(); active.player?.pause?.(); active.player?.remove?.(); } catch (_) {}
    setSpeakingMessageId('');
  };
  const clearTogetherSonicCache = async () => {
    const activeUri = togetherSonicPlayerRef.current?.uri || null;
    const entries = [...togetherSonicCacheRef.current.values()];
    togetherSonicCacheRef.current.clear();
    await stopTogetherSonicSpeech();
    await Promise.all([...new Set([activeUri, ...entries.map((entry) => entry.uri)].filter(Boolean))].map((uri) => FileSystem.deleteAsync(uri, { idempotent: true }).catch(() => {})));
    setError(`Cleared ${entries.length} temporary Together Sonic audio file${entries.length === 1 ? '' : 's'}.`);
  };
  const handleRefreshTogetherSonicVoices = async () => {
    if (!String(togetherApiKey || '').trim()) { setError('Enter a Together AI API key in AI & Prompt Settings before loading Sonic voices.'); return; }
    setIsRefreshingTogetherSonicVoices(true);
    try {
      const voices = await fetchTogetherSonicVoices(togetherApiKey, TOGETHER_SONIC_TTS_MODEL);
      if (!voices.length) throw new Error('Together AI returned no selectable Sonic voices for this account.');
      setTogetherSonicVoices(voices);
      setTogetherSonicVoice((current) => voices.some((voice) => voice.id === current) ? current : voices[0].id);
      setError(`Loaded ${voices.length} Together Sonic voice option(s).`);
    } catch (voiceError) { setError(voiceError.message || 'Together Sonic voices could not be loaded.'); }
    finally { setIsRefreshingTogetherSonicVoices(false); }
  };
  const handleSpeakMessage = async (message) => {
    const messageId = String(message?.messageId || message?.id || '');
    if (!messageId) { setError('This message cannot be identified for speech playback.'); return; }
    if (speakingMessageId === messageId) { await stopTogetherSonicSpeech(); return; }
    try {
      await Speech.stop();
      await stopTogetherSonicSpeech();
      setSpeakingMessageId(messageId);
      const cacheKey = `${messageId}:${togetherSonicVoice}:${TOGETHER_SONIC_TTS_MODEL}:${voiceLocale}:${playbackSpeed}`;
      let cached = togetherSonicCacheRef.current.get(cacheKey);
      if (cached) { try { const info = await FileSystem.getInfoAsync(cached.uri); if (!info?.exists) cached = null; } catch (_) { cached = null; } }
      let uri = cached?.uri;
      if (!uri) {
        const audio = await createTogetherSonicSpeech({ apiKey: togetherApiKey, input: message?.content, voice: togetherSonicVoice, model: TOGETHER_SONIC_TTS_MODEL, locale: voiceLocale });
        uri = `${FileSystem.cacheDirectory}AI_Console_Sonic_${messageId.replace(/[^a-z0-9_-]/gi, '').slice(0,48) || Date.now()}_${Date.now()}.mp3`;
        await FileSystem.writeAsStringAsync(uri, bytesToBase64(audio.bytes), { encoding: FileSystem.EncodingType.Base64 });
        togetherSonicCacheRef.current.set(cacheKey, { uri, lastUsedAt: Date.now() });
        if (togetherSonicCacheRef.current.size > 5) {
          const [evictKey, evict] = [...togetherSonicCacheRef.current.entries()].sort((a, b) => a[1].lastUsedAt - b[1].lastUsedAt)[0] || [];
          if (evictKey && evict?.uri !== uri) { togetherSonicCacheRef.current.delete(evictKey); try { await FileSystem.deleteAsync(evict.uri, { idempotent: true }); } catch (_) {} }
        }
      } else togetherSonicCacheRef.current.set(cacheKey, { ...cached, lastUsedAt: Date.now() });
      const player = createAudioPlayer(uri, { downloadFirst: true });
      player.playbackRate = Math.min(2, Math.max(0.1, Number(playbackSpeed) || 1));
      const active = { player, uri, messageId, subscription: null };
      active.subscription = player.addListener('playbackStatusUpdate', (status) => {
        if (!status?.didJustFinish || togetherSonicPlayerRef.current !== active) return;
        void stopTogetherSonicSpeech();
      });
      togetherSonicPlayerRef.current = active;
      player.play();
    } catch (speechError) {
      await stopTogetherSonicSpeech();
      setError(speechError.message || 'Together Sonic speech playback failed.');
    }
  };
  const handleStopSpeech = async () => { try { await Speech.stop(); await stopTogetherSonicSpeech(); } catch (_) { setError('Unable to stop text to speech.'); } };
