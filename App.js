...
function AIConsoleApp() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (active?.uri) void FileSystem.deleteAsync(active.uri, { idempotent: true }).catch(() => {});
    };
  }, []);
...
${project}`);
  ...
  const stopTogetherSonicSpeech = async () => {
    ...
    try { active.subscription?.remove?.(); active.player?.pause?.(); active.player?.remove?.(); } catch (_) {}
    if (active.uri) { try { await FileSystem.deleteAsync(active.uri, { idempotent: true }); } catch (_) {} }
  ...
  const handleSpeakMessage = async (message) => {
    ...
    try {
      ...
      const uri = `${FileSystem.cacheDirectory}AI_Console_Sonic_${messageId.replace(/[^a-z0-9_-]/gi, '').slice(0,48) || Date.now()}.mp3`;
